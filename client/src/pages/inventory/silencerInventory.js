...
class silencerInventory extends Component {
  constructor(props) {
    super(props);
    this.state = {
      gunData: [],
...
fetch(`/silencers/${this.state.keyParam}/${this.state.valParam}/${sortVar}`)
    .then(res => res.json())
    .then(json => {
  ...
  let sort = "sort";
    ...
    fetch(`/silencers/${keyParam}/${valParam}/${this.state.sort}`)
      .then(res => res.json())
      .then(json => {
        console.log("json", json)
        this.setState({
...
console.log(pageNumbers.length)
    ...
    const items = currentItems.map((item, i) => {
      ...
      <Card key={i} className='inventory-card'>
        ...
        <img className="gun-img" alt={`${item.description1}`}
                // TODO: come up with better way to get images than this solution
                src={`https://www.lipseyscloud.com/images/${item.imagename}`}
                onError={this.usePlaceholderImg}
              />
          <p className="text-center">{item.description1}</p>
          <p className="text-center">{item.description2}</p>
          <h5 className="retail-price text-center">${item.msrp}</h5>
          {item.retailmap > 0 ? (
            <h4 className="text-center">${item.retailmap}</h4>
          ) : (
              <h4 className="text-center">${(item.price * profitMargin).toFixed(2)}</h4>
            )
          }
          {item.total_quantity > 0 ? (
            <h5 className="text-center">{item.quantity} Left</h5>
          ) : (
...
export default silencerInventory;
